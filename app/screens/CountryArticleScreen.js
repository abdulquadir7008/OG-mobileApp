...
const CountryArticleScreen = () => {
    ...
    useEffect(() => {
        fetchCategories();
        fetchData(currentPage, selectedCategoryId); // Fetch data with initial category as null
    }, [selectedCategoryId]);

    useEffect(() => {
        // Check if selectedCategoryId is available
        if (countryId !== null) {
            // Reset posts and fetch data for the selected category
            setPosts([]);
            fetchData(1, selectedCategoryId);
        }
    }, [countryId, selectedCategoryId]);
    ...
    const fetchData = async (page, categoryId = null) => {
        ...
        try {
            ...
            if (countryId == '' || countryId === undefined) {
                apiUrl = `https://opengovasia.com/wp-json/wp/v2/posts/?page=${page}&_embed&per_page=10`;
            } else {
                apiUrl = `https://opengovasia.com/wp-json/wp/v2/posts/?page=${page}&_embed&per_page=10&categories=${countryId}`;
            }
            if (categoryId) {
        ...
        <View>
            ...
            <ScrollView horizontal={false} scrollEventThrottle={400} // Adjust the threshold as needed
                ...
                <View style={styles.viewContent}>
                    ...
                    {/* <Text style={styles.headingTittle}>Our Article</Text> */}
                    ...
                    {categories
                        ...
                        .map((category) => (
                            <Text style={{ marginLeft: 10, marginTop: 10, fontSize: 17, fontWeight: 'bold' }}>{category.name}</Text>
                        ))}
                    {firstPost && (
                    ...
                    {posts
                        ...
                        .map((post, index) => ( // Add index as the second part of the key
                            ...
                            <TouchableHighlight
                                ...
                                underlayColor="#DDD" key={`post-${post.id}-${index}`}>
...
export default CountryArticleScreen;
